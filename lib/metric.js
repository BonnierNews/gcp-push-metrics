...
export default function metric(name, config = {}, logger) {
  ...
  if (predefinedLabels) {
    ...
    combinations.forEach((combo) => {
      ...
      points[key] = createPoint({ labels: combo, value: 0, isPredefined: true });

    });
  } else {
    points[labelsKey()] = createPoint({ labels: null, value: 0, isPredefined: true });
  }
  ...
  const inc = (incLabels, value) => {
    ...
    // TODO:  Still dangerous - e.g. a user sending in 'uuid' => new dynamic metric each time. // e.g. traceId
