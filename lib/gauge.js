...
export default function gauge(config, logger) {
  ...
  const dec = (labels, value) => {
    ...
    if (!baseMetric.points[key]) {
      baseMetric.points[key] = createPoint({ labels, value: 0 });
    }
  ...
  // Delete transient keys to avoid memory leak / cost over time with inactive timeseries
  const intervalReset = () => {
    const points = baseMetric.pointsReferenceFn();
    for (const [ key, val ] of Object.entries(points)) {
      if (val.seriesType !== SERIES_TYPES.CONTINUOUS) {
        delete points[key];
      }
    }
  };
